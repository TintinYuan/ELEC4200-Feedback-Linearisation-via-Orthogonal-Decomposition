...
def orthogonal_component(a_n, A_first_cols):
    """
    Compute the component of a_n orthogonal to the columns of A_first_cols
    """
    v = a_n.copy()
    # for  i in range(A_first_cols.shape[1]):
    #     a_i = A_first_cols[:, i]
    #     proj = (np.dot(a_n, a_i) / np.dot(a_i, a_i)) * a_i
    #     v -= proj
    # return v

    n_cols = A_first_cols.shape[1] if hasattr(A_first_cols, 'shape') else len(A_first_cols)

    for i in range(n_cols):
        a_i = A_first_cols[:, i] if hasattr(A_first_cols, 'shape') else [A_first_cols[i][j] for j in range(len(A_first_cols[0]))]
        dot_an_ai = sum(a_n[j] * a_i[j] for j in range(len(a_n)))
        dot_ai_ai = sum(a_i[j] * a_i[j] for j in range(len(a_i)))
        proj = [(dot_an_ai/dot_ai_ai) * a_i[j] for j in range(len(a_i))]
        v = [v[j] - proj[j] for j in range(len(v))]
...
# orthogonal_vector1 = orthogonal_component(Matrix(vectors[-1]), vectors[:-1])
